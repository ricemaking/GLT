...
class ChargeLineRowViewModel: ObservableObject {
    ...
    func fetchTSCharge() -> TSCharge? {
        ...
        fetchRequest.predicate = NSPredicate(
            format: "employeeID == %d AND chargeID == %d AND month == %d AND year == %d AND day == %d AND saved == NO AND ((offline == NO AND verified == YES AND denied == NO) OR (offline == YES AND verified == NO AND denied == NO)) ",
        ...
        do {
            ...
            if let finalResults = results.first{
                print("charge: \(finalResults.chargeID), day: \(finalResults.day), tempHours: \(finalResults.tempHours ?? 0), version: \(finalResults.version ?? 0), saved: \(finalResults.saved)")
            }
